...
public class PSBT {
    ...
    public void adoptInputs(JSONArray inputs) throws InvalidTransactionException {
        try {
            ...
            if (this.getMySigningInputs().size() == 0) {
                throw new InvalidTransactionException(MainApplication.getApplication().getString(R.string.incorrect_tx_data), "Transaction is not related to this Wallet");
            }
        } catch (JSONException e) {
            e.printStackTrace();
            throw new InvalidTransactionException(MainApplication.getApplication().getString(R.string.incorrect_tx_data), "Data error");
        }
    }
    ...
    public void adoptOutputs(JSONArray outputs) throws InvalidTransactionException {
        try {
            ...
            if (length == 0) {
                throw new InvalidTransactionException(MainApplication.getApplication().getString(R.string.incorrect_tx_data), "Transaction has no output");
            }
            for (int i = 0; i < length; i++) {
